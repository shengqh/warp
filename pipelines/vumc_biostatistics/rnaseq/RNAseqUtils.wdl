...
~{star_path} ~{star_option} \
  --outSAMattrRGline ID:~{sample_name} SM:~{sample_name} LB:~{sample_name} PL:ILLUMINA PU:ILLUMINA \
  --runThreadN ~{cpu} \
  --genomeDir ~{reference_snapshot_path} \
  --readFilesIn ~{left_fq} ~{right_fq} \
  --readFilesCommand "gunzip -c" \
  --outFileNamePrefix ~{sample_name}_ \
  --outSAMtype BAM Unsorted
  ...
  runtime {
    docker: docker
    memory: memory_gb + " GiB"
    disks: "local-disk " + disk_size_gb + " " + (if use_ssd then "SSD" else "HDD") + " " + reference_snapshot_root + " 60 HDD " + reference_snapshot_url
    cpu: cpu
    preemptible: preemptible
  }
...
task star_fusion_SnapShot {
  input {
    ...
    String reference_snapshot_url = "projects/vangard-workflow-data/global/snapshots/starfusion-db-20231029"
    String reference_snapshot_root = "/mnt/disks/star_fusion"
    String reference_snapshot_path = "/mnt/disks/star_fusion"
  ...
  Int disk_size_gb = ceil((fastq_disk_space_multiplier * (size(left_fq, "GB") + size(right_fq, "GB"))) + extra_disk_space)
  ...
  command <<<
    ...
    /usr/local/src/STAR-Fusion/STAR-Fusion \
      --genome_lib_dir ~{reference_snapshot_path} \
      --left_fq ~{left_fq} \
      --right_fq ~{right_fq} \
      --output_dir ~{sample_name} \
      --CPU ~{cpu} \
