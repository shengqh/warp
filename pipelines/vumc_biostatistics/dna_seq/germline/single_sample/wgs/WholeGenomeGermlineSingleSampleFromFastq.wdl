...
workflow WholeGenomeGermlineSingleSampleFromFastq {
  ...
  call AggregatedQC.AggregatedBamQC {
    input:
      base_recalibrated_bam = FastqToAlignedBam.output_bam,
      base_recalibrated_bam_index = FastqToAlignedBam.output_bam_index,
  ...
  call ToCram.BamToCram as BamToCram {
    input:
      input_bam = FastqToAlignedBam.output_bam,
  ...
  call QC.CollectWgsMetrics as CollectWgsMetrics {
    input:
      input_bam = FastqToAlignedBam.output_bam,
      input_bam_index = FastqToAlignedBam.output_bam_index,
  ...
  call ToGvcf.VariantCalling as BamToGvcf {
    input:
      ...
      contamination = FastqToAlignedBam.contamination,
      input_bam = FastqToAlignedBam.output_bam,
      input_bam_index = FastqToAlignedBam.output_bam_index,
      ref_fasta = references.reference_fasta.ref_fasta,
      ref_fasta_index = references.reference_fasta.ref_fasta_index,
  ...
  if (provide_bam_output) {
    File provided_output_bam = FastqToAlignedBam.output_bam
    File provided_output_bam_index = FastqToAlignedBam.output_bam_index
  }

  # Outputs that will be retained when execution is complete
  output {
    Array[File] unsorted_read_group_base_distribution_by_cycle_pdf = FastqToAlignedBam.unsorted_read_group_base_distribution_by_cycle_pdf
    Array[File] unsorted_read_group_base_distribution_by_cycle_metrics = FastqToAlignedBam.unsorted_read_group_base_distribution_by_cycle_metrics
    Array[File] unsorted_read_group_insert_size_histogram_pdf = FastqToAlignedBam.unsorted_read_group_insert_size_histogram_pdf
    Array[File] unsorted_read_group_insert_size_metrics = FastqToAlignedBam.unsorted_read_group_insert_size_metrics
    Array[File] unsorted_read_group_quality_by_cycle_pdf = FastqToAlignedBam.unsorted_read_group_quality_by_cycle_pdf
    Array[File] unsorted_read_group_quality_by_cycle_metrics = FastqToAlignedBam.unsorted_read_group_quality_by_cycle_metrics
