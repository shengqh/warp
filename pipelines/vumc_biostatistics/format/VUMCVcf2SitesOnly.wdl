...
##
## ### Workflow Steps:
## 1. Use GATK MakeSitesOnlyVcf to generate a sites-only version of the input VCF
...
##
## ### Notes:
## - Uses GATK MakeSitesOnlyVcf to remove genotype fields while preserving site information
## - File copy operation to GCP is optional and only executed if a target folder is provided

import "../../../tasks/vumc_biostatistics/GcpUtils.wdl" as GcpUtils

workflow VUMCVcf2SitesOnly {
  input {
    File input_vcf
  ...
  output {
    File output_sites_vcf = select_first([CopyFile.output_file1, Vcf2SitesOnly.output_sites_vcf])
    File output_sites_vcf_index = select_first([CopyFile.output_file2, Vcf2SitesOnly.output_sites_vcf_index])
  }
}
...
gatk --java-options "-Xmx~{command_mem_gb}G" MakeSitesOnlyVcf \
  -I ~{input_vcf} \
  -O ~{output_prefix}.sites.vcf.gz
