...
workflow VUMCPrepareAgdVcfBcftools {
  input {
    File input_vcf
  ...
  if(defined(target_gcp_folder)){
    call GcpUtils.MoveOrCopyTwoFiles as CopyFile {
      input:
        source_file1 = PrepareAgdVcfBcftools.output_vcf,
        source_file2 = PrepareAgdVcfBcftools.output_vcf_index,
        is_move_file = false,
  ...
  output {
    File output_vcf = select_first([CopyFile.output_file1, PrepareAgdVcfBcftools.output_vcf])
    File output_vcf_index = select_first([CopyFile.output_file2, PrepareAgdVcfBcftools.output_vcf_index])
    Int num_samples = PrepareAgdVcfBcftools.num_samples
    Int num_variants = PrepareAgdVcfBcftools.num_variants
  }
}

task PrepareAgdVcfBcftools {
  input{
    File input_vcf
    File input_vcf_index
...
bcftools view --threads ~{cpu} -f PASS -Oz -o ~{target_vcf} ~{input_vcf}
